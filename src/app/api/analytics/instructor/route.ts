...
export async function GET(request: NextRequest) {
  try {
    ...
    const totalSessions = instructorTracks.reduce((sum, track) => 
      sum + track.liveSessions.length, 0
    );

    const totalScheduledSessions = instructorTracks.reduce((sum, track) => 
      sum + track._count.liveSessions, 0
    );

    const completedSessions = instructorTracks.reduce((sum, track) => 
      sum + track.liveSessions.filter(s => s.status === "completed").length, 0
    );

    const totalAttendances = instructorTracks.reduce((sum, track) => 
      sum + track.liveSessions.reduce((sessionSum, session) => 
        sessionSum + session.attendances.length, 0
      ), 0
    );

    const totalPossibleAttendances = instructorTracks.reduce((sum, track) => 
      sum + track.liveSessions.reduce((sessionSum, session) => 
        sessionSum + session.attendances.length, 0
      ), 0
    );
    ...
    const sessionCompletionRate = totalScheduledSessions > 0 
    ...
    const trackAnalytics = instructorTracks.map(track => {
      ...
      const completedTrackSessions = sessions.filter(s => s.status === "completed").length;
      const avgAttendancePerSession = sessions.length > 0 
        ? sessions.reduce((sum, s) => sum + s.attendances.length, 0) / sessions.length 
        : 0;
      ...
      return {
        ...
        completionRate: totalTrackSessions > 0 ? (completedTrackSessions / totalTrackSessions) * 100 : 0,
    ...
    // Attendance trends
    const attendanceTrends = Object.keys(sessionsByWeek).map(date => {
      const sessionsOnDate = instructorTracks.reduce((acc, track) => {
        const dateSessions = track.liveSessions.filter(s => 
          s.date.toISOString().substr(0, 10) === date
        );
        return acc.concat(dateSessions);
      }, []);

      const totalAttendanceOnDate = sessionsOnDate.reduce((sum, session) => 
        sum + session.attendances.filter(a => a.status === "present").length, 0
      );

      const totalPossibleOnDate = sessionsOnDate.reduce((sum, session) => 
        sum + session.attendances.length, 0
      );
      ...
      return {
        ...
        attendanceRate: totalPossibleOnDate > 0 ? (totalAttendanceOnDate / totalPossibleOnDate) * 100 : 0
      };
    ...
    const effectivenessScore = effectivenessFactors.reduce((sum, factor) => sum + factor, 0) / effectivenessFactors.length;
    ...
    const analytics = {
      ...
      studentPerformance: {
        ...
        averageAttendance: Math.round(
          studentPerformance.reduce((sum, s) => sum + s.attendancePercentage, 0) / 
