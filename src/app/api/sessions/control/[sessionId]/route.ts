...
export async function PUT(
  ...
  try {
    ...
    if (session.user.role === "instructor" && liveSession.instructorId !== session.user.id) {
      return NextResponse.json({ error: "You can only control your own sessions" }, { status: 403 });
    }
    ...
    const updatedSession = await prisma.liveSession.update({
      ...
      data: {
        status: newStatus,
        notes: notes ? `${liveSession.notes || ""}\n[${new Date().toISOString()}] ${session.user.name}: ${notes}`.trim() : liveSession.notes
      },
      include: {
        track: {
          include: {
    ...
    if (action === "start") {
      ...
      const newStudents = enrolledStudents.filter(s => !existingStudentIds.includes(s.id));
...
export async function GET(
  ...
  try {
    ...
    const canControl = session.user.role === "instructor" && liveSession.instructorId === session.user.id ||
                      ["coordinator", "manager", "ceo"].includes(session.user.role);

    // Get available actions based on current status
    const currentStatus = liveSession.status;
    const availableActions: Record<string, Array<{action: string, label: string, color: string}>> = {
      scheduled: [
    ...
    let duration = null;
    if (liveSession.status === "in_progress" || liveSession.status === "completed") {
    ...
    return NextResponse.json({
      ...
      attendanceStats: {
        ...
        present: liveSession.attendances.filter(a => a.status === "present").length,
        absent: liveSession.attendances.filter(a => a.status === "absent").length,
