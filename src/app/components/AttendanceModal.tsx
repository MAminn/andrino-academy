...
export default function AttendanceModal({
  ...
  const handleSave = async () => {
    try {
      ...
      const attendanceUpdates = Object.entries(attendanceData).map(
        ([studentId, data]) => ({
          studentId,
          status: data.status,
          notes: data.notes,
  ...
  const filteredAttendances =
    session?.attendances.filter((attendance) => {
      if (filterStatus === "all") return true;
      return (
        (attendanceData[attendance.studentId]?.status || attendance.status) ===
        filterStatus
      );
    }) || [];
            ...
            <div className='flex-1 overflow-auto p-6'>
              <div className='space-y-3'>
                {filteredAttendances.map((attendance) => {
                  const currentStatus =
                    attendanceData[attendance.studentId]?.status ||
                    attendance.status;
                  const currentNotes =
                    attendanceData[attendance.studentId]?.notes ||
                    attendance.notes ||
                    "";
