...
export default function StudentDashboard() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const upcomingResponse = await fetch(`/api/sessions?startDate=${today}&endDate=${nextMonth.toISOString().split('T')[0]}&studentId=${session?.user?.id}`);
        ...
        const attendanceResponse = await fetch(`/api/attendance?studentId=${session?.user?.id}`);
  ...
  const totalSessions = studentData?.grade?.tracks?.reduce((sum, track) => sum + track._count.liveSessions, 0) || 0;
  const attendedSessions = attendanceHistory.filter(record => record.status === 'present').length;
      ...
      {!studentData?.grade && (
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-6">
          <div className="flex items-center">
            ...
            <div>
              <h3 className="text-yellow-800 font-medium">في انتظار تعيين المستوى الدراسي</h3>
              <p className="text-yellow-700 text-sm mt-1">
                يرجى التواصل مع الإدارة لتعيينك في المستوى الدراسي المناسب للوصول إلى المسارات والجلسات.
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="space-y-4">
            ...
            {studentData.grade.tracks.length === 0 && (
              <div className="text-center py-8">
                ...
                <p className="text-gray-500">لا توجد مسارات متاحة في مستواك حالياً</p>
                <p className="text-sm text-gray-400 mt-2">سيتم إضافة المسارات قريباً</p>
              </div>
            )}
          </div>
        </QuickActionCard>
      ...
      <QuickActionCard title='الجلسات القادمة'>
        {upcomingSessions.length === 0 ? (
          <div className="text-center py-8">
            ...
            {!studentData?.grade && (
              <p className="text-sm text-gray-400 mt-2">سيتم عرض الجلسات بعد تعيين المستوى الدراسي</p>
            )}
          </div>
        ) : (
          <div className="space-y-4">
      ...
      <QuickActionCard title='سجل الحضور'>
        {attendanceHistory.length === 0 ? (
          <div className="text-center py-8">
            ...
            <p className="text-sm text-gray-400 mt-2">سيظهر سجل حضورك بعد حضور الجلسات</p>
          </div>
        ) : (
          <div className="space-y-4">
